...
import { X, Check, ChevronsUpDown, FileText, ExternalLink, Trash2, Upload, Wand2, CalendarIcon, Loader2 } from "lucide-react";
...
export function WorkflowExpensePanel({ file, existingTransaction, onClose, onSaved }: WorkflowExpensePanelProps) {
  ...
  // Projetos: só após sessão (RLS auth.uid()); select mínimo evita falhas se colunas diferirem no remoto
  const {
    data: expenseProjects,
    isLoading: projectsLoading,
    isError: projectsQueryError,
    error: projectsQueryErr,
  } = useQuery({
    queryKey: ["expense-projects-active", user?.id],
                  ...
                  >
                    ...
                    <PopoverContent className="w-[min(100vw-2rem,320px)] p-0" align="start">
                      <div className="flex flex-col gap-1 p-2">
                        ...
                        {projectsQueryError && (
                          <p className="text-xs text-destructive px-1 py-2">
                            {projectsQueryErr instanceof Error
                              ? projectsQueryErr.message
                              : "Erro ao carregar projetos."}
                          </p>
                        )}
                        {!projectsLoading && !projectsQueryError && (
                              ...
                              {(projectsVisibleForCompany ?? [])
                                ...
                                .map((project) => (
                                  <button
                                    ...
                                    type="button"
                                    className={cn(
                                      "flex w-full min-w-0 items-center rounded-sm px-2 py-1.5 text-sm text-left hover:bg-accent",
                                      watch("project_id") === project.id && "bg-accent"
                                    )}
                                    onClick={() => {
