...
function handleWorkFilesDelete(req, res) {
  const url = new URL(req.url, `http://${HOST}`);
  const filePath = url.searchParams.get("file") || "";
  ...
  try {
    ...
    const stat = fs.statSync(resolved);
    if (!stat.isFile()) {
      jsonResponse(res, 400, { error: "Caminho não é um ficheiro" });
      return;
    }
...
const server = http.createServer((req, res) => {
  ...
  } else if (url.pathname === "/api/work-files/delete" && req.method === "DELETE") {
    handleWorkFilesDelete(req, res);
